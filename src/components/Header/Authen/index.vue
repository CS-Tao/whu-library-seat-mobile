<template>
    ...
    <div class="auth-tip">
      ...
      <i class="button-icon" :style="ghBtnloading?'height:auto;':null">
    ...
    <span v-if="socketFailMessage" class="socket-err-msg">{{socketFailMessage}}</span>
      ...
      <el-input v-model="inputToken" placeholder="Here to paste your token" style="margin-top: 8px;"/>
      ...
      <i class="operation-icon" @click="$openLink('https://github.com/CS-Tao/whu-library-seat-mobile')">
  ...
  methods: {
    ...
    connectSocket () {
      ...
      this.socket.on('socketId', (socketId) => {
        this.$openLink(`${socketBaseUrl}/comfirm?socketid=${socketId}&device=mobile`)
        this.workMode = workFlowModes.waitConfirm
      })
