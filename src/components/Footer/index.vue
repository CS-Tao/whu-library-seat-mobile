<template>
    ...
    <div class="warper flex-col" @click.stop="openMainBody()">
      ...
      <span style="cursor: default!important;" v-if="hasToken">&nbsp;&nbsp;&nbsp;</span>
      <i v-if="hasToken" class="operation-icon" @click.stop="logout()" style="margin-right: 8px;">
...
export default {
  ...
  computed: {
    ...
    newVersion () {
      if (this.checkInfo && this.checkInfo.version) {
        return this.checkInfo.version
      } else {
        return null
      }
    },
    newVersionFilePath () {
      if (this.checkInfo && this.checkInfo.file) {
        return this.checkInfo.file
      } else {
        return null
      }
    },
    updateAvailable () {
      return this.newVersion !== null && this.currentVersion !== this.newVersion
    }
  },
  mounted () {
  ...
  methods: {
    openSource () {
      this.$openLink('https://github.com/CS-Tao/whu-library-seat-mobile')
    },
    openDocument () {
    ...
    downloadUpdate () {
      if (this.newVersionFilePath) { this.$openLink(this.newVersionFilePath) }
    },
    exitApp () {
      this.$exitApp()
    }
