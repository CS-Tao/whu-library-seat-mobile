<template>
    ...
    <el-form v-show="showMode==='normal'" :model="form" ref="seatForm" class="form">
      <div style="margin:auto;text-align:center;padding:3vh 0;">
        ...
        <el-button type="primary" class="main-button" @click="mainButtonClicked()">
          {{ functionMode }}
        </el-button>
      </div>
    </el-form>
    ...
    <monitor-form
      v-if="hasToken&&monitorActive"
      :room="singleLibRooms.filter(room => room[0] === form.room)"
      :date="freeDates[0] === form.date
        ? ('今天' + ' - ' + getWeekDay(freeDates[0]))
        : ('明天' + ' - ' + getWeekDay(freeDates[1]))"
      :beginTime="freeBeginTime.filter(time => time.time === form.beginTime)"
      :endTime="freeEndTime.filter(time => time.time === form.endTime)"
      :battery="form.battery"
      :window="form.sun"
      :okBtnText="monitorOkBtntext"
      :cancelBtnText="monitorCancelBtntext"
      :workingStatus="monitorStatus"
      :isWorking="isMonitorWorking"
      @monitorFunc="monitor()"
      @btnClick="monitorFormBtnClicked($event)">
    </monitor-form>
	</div>
</template>
...
export default {
  ...
  computed: {
    ...
    functionMode () {
      switch (this.form.seatNum) {
        case -1:
          return this.functionModes.monitor
        default:
          return this.functionModes.timer
      }
    }
  },
  mounted () {
  ...
  methods: {
    ...
    mainButtonClicked () {
      ...
      switch (this.functionMode) {
        case this.functionModes.monitor:
          // 打开捡漏监视器
    ...
    updateSeatsStatus (successCB = null, failCB = null) {
      // successCB(data) 成功回调
      // failCB(e) 失败回调
      ...
      libraryRestApi.SearchSeat(
        ...
        if (response.data.status) {
          ...
          if (successCB) {
            successCB(this.seatsForSelectIsAvailable)
          }
        } else {
          this.seatsForSelectIsAvailable = null
          if (failCB) {
